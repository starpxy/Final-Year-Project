...
class LogWriter:
    ...
    def write_error_log(self, content):
        if not os.path.exists("logs"):
            os.mkdir("logs")
        logging.basicConfig(format='%(asctime)s - %(levelname)s - %(message)s', filename="logs/error.log", filemode="a",
                            level=logging.ERROR)
    ...
    def write_info_log(self, content):
        if not os.path.exists("logs"):
            os.mkdir("logs")
        logging.basicConfig(format='%(asctime)s - %(levelname)s - %(message)s', filename="logs/info.log", filemode="a",
                            level=logging.INFO)
