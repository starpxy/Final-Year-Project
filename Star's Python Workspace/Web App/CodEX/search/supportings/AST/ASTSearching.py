...
class ASTSearching:
    __pickle_path = configs['paths']['AST_indexing_path']
    ...
    def search(self, query, matchingLines):
        ...
        if os.path.exists(self.__pickle_path):
            rfile = open(self.__pickle_path, 'rb')
        ...
        for w in qTree:
            if isinstance(w, tuple):
                ...
                if not find and qTree[w] is not None:
                    if len(qTree[w]) > 0:
                        self.similarities(qTree[w], hashTrees, weights, similarities, maxWeight, qLineNums,
