...
class JavaAST():
            ...
            if hasattr(root, 'attrs'):
                ...
                for a in root.attrs:
                    ...
                    if a is not 'documentation':
                        ...
                        if a is 'name' and ((not isinstance(root, javalang.tree.ReferenceType) and not isinstance(root,
                                                                                                                  javalang.tree.ReferenceType)) or v in names):
                            if v not in names:
                                names.append(v)
                            # print('================')
                            # print((type(root),a))
                        ...
                        # ignore values like strings, numbers, booleans, null
                            ...
                            if not isinstance(v, list):
                                if isinstance(v, javalang.tree.MethodInvocation) and hasattr(v,
                                                                                             'attrs') and v.__getattribute__(
    ...
    def getResults(self, query, page):
        ...
        results = Results.Results(numOfResults=length, matchingLines=matchingLines,
                                  globalSimilarity=globalSimilarity, matchingBlocks=matchingBlocks,blockWeights=self.blockWeights)
        disMatchingBlocks = []
        disPlagiarismList = []
        disDocumentList = []
